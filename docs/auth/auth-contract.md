...
sequenceDiagram
    ...
    participant C as Expo App (iOS, SDK 54)
    participant KP as Kratos Public (auth.cvera.app)
    participant KA as Kratos Admin (kratos:4435)
    participant A as EVT Go API (api.cvera.app)
    participant D as EVT DB (evt.refresh_tokens)
    ...
    Note over C: App now has a valid Kratos session. Next: exchange for API tokens.

    C->>A: HTTPS POST /auth/exchange { kratos_session_token, device_id? }
    A->>KA: HTTP GET /sessions/whoami or /sessions/{id} (using kratos_session_token)
    ...
    A->>D: INSERT refresh_tokens (sha256(refresh_token), user_id, device_id, expires_at)

    D-->>A: OK

    A-->>C: 200 OK { access_token, refresh_token, user }

    Note over C: Store refresh_token in SecureStore. Keep access_token in memory only.

    C-->>U: User is now signed in to EVT API (Case list, evidence, approve/reject)
```
---
